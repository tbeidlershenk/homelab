...
if [ ! -d "$API_VENV_DIR" ]; then
    ...
    log "Created API virtual environment."
else
    log "API virtual environment exists."
fi
"$API_VENV_DIR/bin/pip" install --upgrade pip
"$API_VENV_DIR/bin/pip" install -r "$BASE_DIR/api/requirements.txt"
systemctl enable --now api
# systemctl restart api
log "Maintenance API service running."
